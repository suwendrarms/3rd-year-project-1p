<header class="navbar navbar-fixed-top bg-system">
    ...
    <ul class="nav navbar-nav navbar-right">
        ...
        <li class="dropdown dropdown-fuse">
            <a href="/photos" class="dropdown-toggle fw600" data-toggle="dropdown">
                <span class="hidden-xs"><name>{{\Auth::user()->name}}</name> </span>
                <span class="fa fa-caret-down hidden-xs mr15"></span>
                @if(\Auth::user()->employee->photo)
                    <img src="{{\Auth::user()->employee->photo}}" width="50px" height="50px" alt="" class="mw55">
                @else
                <img src="{{ URL::asset('assets/img/avatars/profile_pic.png') }}" alt="" class="mw55">
                    @endif
            </a>
            </a>
